...
loss_function=nn.CrossEntropyLoss(weight=torch.tensor([4,2,2,2,2],dtype=torch.float),reduction='mean')   # 第一个类别（评分1）比较少，加大一下权重，而且它确实影响恶劣一些
...
def write_log(w):
    file_name = 'model/data/data' +"_{}.log".format("deepfm")
...
def train_and_eval(model, train_loader, eval_dataloader, epochs, device):
    ...
    for _ in range(epochs):
        ...
        if cur_loss < best_loss:
            best_loss = cur_loss
            torch.save(model.state_dict(), "model/data/deepfm_best.pth")
